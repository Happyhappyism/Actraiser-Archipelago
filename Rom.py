...
def patch_rom(world: World, rom: ActraiserProcedurePatch):
    ...
    def randomize_lairs():
        ...
        rom.write_bytes(0x639C0, bytes(custom_lair_images[picked_lair_img][0]))
        rom.write_bytes(0x63BC0, bytes(custom_lair_images[picked_lair_img][1]))
        ...
        for object in pal_sources:
            ...
            for clrinx in pal_sources[object][1]:
                if clrinx == 18:
                    ...
                    rom.write_bytes(object+i,bytes(color))
                elif clrinx == 16:
                    i = i+2
                    continue
                elif clrinx == 17:
                    rom.write_bytes(object+i,bytes([0x00,0x00]))
                else:
    ...
    rom.write_bytes(0x12E0,  bytes([0xE0, 0x60, 0x13])) #CPX #$1360
    ...
    rom.write_bytes(0x37, bytes([0x22, 0x00, 0x88, 0x1F]))  #JSL $1F8800
    
    #Jump to Item handling routine
    rom.write_bytes(0x12C69, bytes([0x22, 0x10, 0x80, 0x1F]))  #JSL $1F8010 (from civ segments)
    rom.write_bytes(0xDB, bytes([0x22, 0x60, 0x80, 0x1F]))  #JSL $1F8060 (from action stages)
    ...
    rom.write_bytes(0x892D, bytes([0x20, 0xEA, 0x90 ])) #JSR $90EA

    #for levelup routine
    rom.write_bytes(0x1B3BD, bytes([0xAF, 0x40, 0x1A, 0x7E ])) #LDA $1A40

    #levelup checks
    rom.write_bytes(0x1B3D4, LEVELUP_CHECKS )

    # skip master being at max level check
    rom.write_bytes(0x1E417, bytes([0xCF, 0x40, 0x1A, 0x00])) # CMP $0291 -> CMP $001A40
    ...
    #Jump to send death
    rom.write_bytes(0x1CE8, bytes([0x22, 0xC0, 0x80, 0x1F, 0xEA, 0xEA]))
    ...
    rom.write_bytes(0x8D00, bytes([0x22, 0x42, 0x83, 0x1F])) #JSL $1F8342
    ...
    rom.write_bytes(0x9CB4, bytes([0xBF, 0xFF]))
    rom.write_bytes(0xF0AE, bytes([0xC5, 0xFF]))
    ...
    rom.write_bytes(0x922B, bytes([0x22, 0x40, 0x86, 0x1F, 0xEA])) # JSL $1F8640
    ...
    if world.options.fire_sword:
        #rom.write_bytes(0x8CA, bytes([0xEA, 0xAD, 0x4A, 0x1A])) #LDA $1A4A
        rom.write_bytes(0x8CA, bytes([0x22, 0xB0, 0x82, 0x1F])) # JSL 1F82B0

    #Jump to arrow power routine
    rom.write_bytes(0xB361, bytes([0x22, 0x58, 0x87, 0x1F, 0xEA ]))

    rom.write_bytes(0xF8758, bytes([
    ...
    rom.write_bytes(0x90E5, bytes([0x22, 0x80, 0x87, 0x1F, 0x60])) # JSL $1F8780 -> RTS

    #jump to source of life check routine
    rom.write_bytes(0x9CBD, bytes([0x22, 0x6E, 0x87, 0x1F, 0x80, 0x03, 0xEA, 0xEA, 0xEA,  ]))# JSL $1F876E (5 NOPS for padding)
    ...
    #rom.write_bytes(0x181B8, bytes([0x20, 0x8E, 0x80 ])) #JSR $B701

    #Do not gain civ levels
    rom.write_bytes(0x1808C, bytes([0x80, 0x05, 0x22, 0x9A, 0xFF, 0x01, 0x60,]))
    rom.write_bytes(0x1B6FC, bytes([
        0x22, 0xA0, 0x87, 0x1F, 
        0xAE, 0xFB, 0x7B, 
        0x80, 0x2C,
    ...
    rom.write_bytes(0x16DF, bytes([0x22, 0xE0, 0x87, 0x1F ])) # JSL $1F87E0
    ...
    #Not currently used
    rom.write_bytes(0xFF9A, bytes([0x20, 0x29, 0x8E, 0x6B])) #JSR $8E29 -> RTL 


    #Text code
    #'Master, you do not \n have this yet.'
    rom.write_bytes(0xF283, bytes([0x8A, 0x80, 0xFB, 0xC9, 0x0D, 0x89, 0x87, 0x79, 0x65, 0x74, 0x2E, 0x00]))

    #'AW $1A45'
    rom.write_bytes(0xF4A2, bytes([0x41, 0x57, 0x40, 0x09, 0x02, 0x45, 0x1A, 0x20]))
    #'Crystal $1A48'
    rom.write_bytes(0xF4BF, bytes([0x43, 0x72, 0x79, 0x73, 0x74, 0x61, 0x6C, 0x20, 0x09, 0x02, 0x48, 0x1A]))
    # Level Select Text
    rom.write_bytes(0xF783, LEVEL_TEXT)

    #Recieve Item Text
    rom.write_bytes(0xF9A00, bytes([0x53, 0x69, 0x72, 0x2C, 0x40, 0x57, 0x65, 0x40, 0x00])) #Sir, We 
    ...
    rom.write_bytes(0x1C025, bytes([0x22, 0xA0, 0x86, 0x1F ])) # JSL $1F86A0

    # Max Town Population Checks
    rom.write_bytes(0xF86A0, MAX_POP_LOCS)

    # Pyramid Check Fix???
    rom.write_bytes(0x1FBEA, bytes([0xA0, 0x60, 0xFE])) #LDY #$FE60
    rom.write_bytes(0x1FE60, bytes([0x40, 0x92]))

    # Bridge Food Increase
    rom.write_bytes(0x1C0E5, bytes([0x30])) # LDY #$30

    # Randomized level entry requirements
     # Repoint level entry text
    base_stage_req_tex = 0xEFE8
    base_stage_req_pnt = 0xFFA4
    for i in range(0,6):
        text_pnt = int.to_bytes(base_stage_req_pnt + (i * 3),2,"little")
        rom.write_bytes(base_stage_req_tex + (i * 2), text_pnt)
    #rom.write_bytes(0xEFE8, bytes([0xC4,0xFF,0xC7,0xFF,0xCA,0xFF,0xCD,0xFF,0xD0,0xFF,0xD3,0xFF]))
    act_levels = world.act_levels
    for x in range(6):
        rom.write_bytes(0x8773 + (x*2), bytes([act_levels[x] + 1]))
        if act_levels[x] >= 9:
            textlevel = bytes([0x31, ((act_levels[x]-9) + 0x30)])
        else:
            textlevel = bytes([(act_levels[x] + 0x31), 0x00])
        rom.write_bytes(base_stage_req_pnt + (x*3), textlevel)
        rom.write_bytes(base_stage_req_pnt + (x*3)+2, bytes([0x00]))
    ...
    #AP Ofering Icons
    rom.write_bytes(0xF0AC, bytes([0xA0, 0xFF])) # Use Bag Icon Pointer
    rom.write_bytes(0xFFA0, bytes([0x45, 0x05])) # Use Bag Icon For Item 10
    #AP Icon Top Tiles
    ...
    rom.write_bytes(0xB63E, bytes([0x22, 0x30, 0x83, 0x1F ])) #JSL $1F8330
    ...
    rom.write_bytes(0x4F6, bytes([0x22, 0xA0, 0x83, 0x1F ])) #JSL $1F83A0
    rom.write_bytes(0x1AA10, bytes([0x22, 0x20, 0x84, 0x1F ])) #JSL $1F8420
    ...
    rom.write_bytes(0x22A4, bytes([0x22, 0x80, 0x86, 0x1F, 0xEA])) #JSL 1F8680

    #Check high score and write action score checks
    rom.write_bytes(0xF8680, SCORE_LOCS)

    #Free space
    rom.write_bytes(0x86E5, bytes([0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA]))

    #Called when clearing a level, free to JSL with 3 free extra bytes above it
    rom.write_bytes(0x2731, bytes([0xEA, 0xEA, 0xEA, 0xEA]))

    # Goal routines
    rom.write_bytes(0x75A9, bytes([0x20, 0x23, 0xFF])) #JSR $FF20
    rom.write_bytes(0x2357, bytes([0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA, 0xEA,0xEA, 0xEA, 0xEA])) # Do not open Death heim
    ...
    rom.write_bytes(0x18E2B, bytes([0x22, 0x00, 0x83, 0x1F ])) #JSL $1F8300
    ...
    if world.options.fast_construct:
        rom.write_bytes(0x181E0, bytes([0xC9, 0x80, 0x01])) #change Construction Wait Timer

    #DEBUG CODE
    #if world.options.debug_mode:
    #    rom.write_byte(DEBUG_ADR, 0x01)
    #    rom.write_bytes(0x181E0, bytes([0xC9, 0x30, 0x00])) #change Construction Wait Timer
    #else:
    ...
    if world.options.population_goal:
        ...
        rom.write_bytes(POPGOAL_ADR, bytes([0x7F, 0x7F])) #Unachieveable
